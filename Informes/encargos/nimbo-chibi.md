---
description: Arregla lo que se ve mal de cerca en los muñecos de Isla Nimbo: el pelo es un cacillo abierto y la cara un parche de una sola cara.
mode: all
...
La cámara pasó a tercera persona esta madrugada: de 15 m y 48° a **5,5 m y 18°**. Los
vecinos ya no se ven desde arriba y lejos, se ven **de cerca y por detrás**. Y ahí sale lo
que desde 150 m nunca se vio (`Informes/nimbo-informe-camara.md`):

1. **El casquete del pelo es un cacillo abierto.** `MeshShapes.SphericalCap` (`:74-114`)
   genera una tapa sin cerrar: desde abajo se le ve el borde en canto, como papel.
2. **La cara es un parche de una sola cara al frente.** `ChibiMeshBuilder.BuildFaceQuad`
   (`:352-395`), pegado a la esfera cerrada de la cabeza. De frente y de perfil se ve; el
   problema es el canto y los ángulos rasantes, que ahora son constantes.

Con la cámara detrás del protagonista, **los vecinos que tienes delante se ven así todo el
rato**.

# Tu carpeta

`Assets/_Project/Scripts/Art/Chibi/**`.

Está libre desde hace poco: el agente `nimbo-ropa` acaba de terminar ahí y su trabajo está
aprobado. **Léelo antes de tocar** (`OutfitLook.cs`, `IslanderView.cs`, y su parte de
`ChibiMeshBuilder`) para no deshacerle nada — y si algo tuyo choca con lo suyo, dilo en el
informe en vez de decidirlo por tu cuenta.

# Cómo se hace aquí

**Todo el arte se genera por código.** `MeshShapes` tiene esfera, cilindro, caja, cápsula
y casquete. Cerrar un casquete es geometría, no un asset. Y mira `RockMeshBuilder`, que
resuelve el sombreado plano por cara, y `FoliageMeshBuilder`, que hace el truco de
unificar normales: las dos técnicas pueden servirte.

# Cuidado con dos cosas

**El coste.** Hay hasta doce vecinos en pantalla más el protagonista. Si cierras el
casquete duplicando geometría, di cuántos triángulos añades por cabeza.

**No arregles lo que no está roto.** El informe de cámara dice que **todo lo demás cierra**
—cajas de 6 caras, cilindros con tapas, esferas, cápsulas—. Son estos dos y nada más.

# Comprobar

Esto se juzga **mirándolo**. Añade a `Tests/PlayMode/CapturaEstilo.cs` un encuadre de un
vecino visto desde detrás y desde abajo, que es donde se delata — y **no borres los siete
que ya hay**, los pusieron otros agentes. Deja escrito en el informe qué captura hay que
mirar y qué se debería ver.

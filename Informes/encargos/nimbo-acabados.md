---
description: Enciende los acabados de vivienda de Isla Nimbo: hay 40 papeles y suelos con precio que nadie puede comprar, elegir ni ver.
mode: all
...
`SetWallpaper` está en el contrato (`IHousingService.cs:55`) y en la implementación
(`HousingService.cs:149`) **sin un solo llamador**. `InteriorView` pinta colores fijos
(`InteriorView.cs:192,200`) y nunca consulta el id guardado.

Y encima **los ids por defecto no existen en el catálogo**: `wall_liso_crema`
(`RoomLayout.cs:68-69`) y `floor_madera_clara` (`RoomLayout.cs:96`,
`HomeUpgradeService.cs:274`, `InteriorView.cs:127`) frente a los 20+20 acabados reales de
`catalogo_acabados.json`. Es la enfermedad del enchufe en un agujero que no existe, otra
vez.

Resultado: **40 acabados con precio y categoría vendible que nadie puede usar**, y
`FurnishPanel` además los excluye a propósito (`FurnishPanel.cs:104`).

# Tu carpeta

`Assets/_Project/Scripts/Housing/**`, `Assets/_Project/Scripts/Art/World/InteriorView.cs`
y `Assets/_Project/Scripts/UI/Player/FurnishPanel.cs`.

# El orden

1. **Los ids por defecto primero.** Que `RoomLayout` y `HomeUpgradeService` arranquen con
   acabados que existan de verdad en el catálogo. Sin esto lo demás no se sostiene.
2. **Que `InteriorView` lea el id guardado** en vez de pintar color fijo. Aquí está el
   trabajo de verdad: el acabado tiene que verse.
3. **Que se puedan elegir** desde `FurnishPanel`, que hoy los excluye.

Ojo: las tiendas acaban de arreglarse en otra rama del trabajo y ahora sí venden. Si tu
categoría de acabados encaja en una tienda existente, dilo en el informe — pero **no
toques `Scripts/Economy/`**, que es de otro.

# Comprobar

Una prueba que recorra los ids por defecto y exija que existan en el catálogo — la que
habría cazado esto. Y una de juego que entre en casa, cambie un acabado y compruebe que
`InteriorView` lo refleja.

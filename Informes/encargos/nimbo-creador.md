---
description: Da voz y ropa al creador de personajes de Isla Nimbo: el GDD lo marca completo y no tiene ninguna de las dos.
mode: all
...
El GDD §17.1 dice `[x] Creador de personajes completo (cuerpo, cara, voz, ropa, personalidad)`.
Hay cuerpo, cara, personalidad y nombre (`CreatorPanel.cs:238-266,163-166,61`). **No hay
pestaña de voz ni de ropa**: un grep de `Voz|Timbre|ropa|outfit` en `CreatorPanel` sale vacío.

Y lo que duele: **la voz ya existe entera**. `VoiceConfig` está en `IslanderIdentity.cs:10`,
la sortea `IslanderFactory.cs:41` y la sintetiza `AudioDirector.cs:361`. Cada habitante
tiene su timbre desde el primer día — el jugador simplemente no lo elige nunca.

La ropa está en `catalogo_ropa.json` (60 prendas, 4 a nivel 1) y **desde esta madrugada
por fin se renderiza en el muñeco** (mira `OutfitLook.cs` e `IslanderView.cs`, trabajo del
agente nimbo-ropa). Elegirla en el creador tiene sentido por primera vez.

# Tu carpeta

`Assets/_Project/Scripts/UI/Creator/**`.

Si necesitas que la fábrica acepte una voz o una prenda elegidas en vez de sortearlas,
**describe el enganche** —fichero, línea, firma— y lo aplica el orquestador.
`IslanderFactory` no es tuyo.

# Lo que quiero

**La voz que se pueda oír antes de elegirla.** Un selector de timbre que no suene es un
menú de nombres. Mira cómo `AudioDirector` sintetiza la voz y busca la forma de disparar
una muestra desde el creador — si el camino pasa por el `EventBus`, mejor.

**La ropa que se vea en el muñeco del creador mientras la eliges.** El creador ya enseña
una vista previa del personaje; que la prenda se aplique ahí es lo que convierte la
elección en una decisión y no en una lista.

**Solo lo de nivel 1.** Hay 4 prendas desbloqueadas al empezar de 60: el creador no puede
ofrecer las 60 o rompe la progresión.

# Cuidado con

El creador es también **la primera pantalla de una partida nueva** y ya tiene scroll y 25
deslizadores. Dos pestañas más pueden convertirlo en un formulario. Piensa cuántos pasos
aguanta antes de que alguien quiera saltárselo, y dilo en el informe aunque decidas
añadirlas igual.

# Comprobar

Una prueba de juego que abra el creador, elija una voz y una prenda, termine, y compruebe
que **el habitante creado las lleva** — no que el panel las mostró. Si solo compruebas el
panel, no has comprobado nada.
